...
$slmt_num_sg_x = !calc($HEAD_SIZE 32 /)
$slmt_num_sg_y = !calc($BLOCK_SIZE 32 /)
$slmt_sgs = 16
$slmt_wg_size_x = !calc($slmt_num_sg_x $slmt_sgs *)
$slmt_wg_size_y = $slmt_num_sg_y
$slmt_attr = {subgroup_size=$slmt_sgs, work_group_size=[$slmt_wg_size_x,$slmt_wg_size_y]}

; %Q: (d,N,H,B)
; %K: (d,N,H,B)
; %V: (d,N,H,B)
; %O: (d,N,H,B)
func @flash_attention_slmt(%Q: memref<f16x$HEAD_SIZE x?x?x?,strided<1,?,?,?>> {stride_gcd=[1,64,64,64]},
    ...
    %cnum_subgroups_x = constant $slmt_num_sg_x : index
    ...
    %P = alloca {alignment=64} : memref<f16x32x32x $slmt_num_sg_x x $slmt_num_sg_y,local>
    ...
    parallel {
        ...
        %o_acc,%maxvec_acc = for %n=%c0,%seq_len,%n_step
            ...
            %shat = for %m=%c0,%chead_size,%c32 init(%s_iter=%s_init) -> (coopmatrix<f32x32x32,matrix_acc>) {
                %kk = cooperative_matrix_load %k[%m,%n_kq] : coopmatrix<f16x32x32,matrix_b>
                %qq = cooperative_matrix_load.t %q[%m,%j0] : coopmatrix<f16x32x32,matrix_a>
            ...
            %o_update = for %m=%c0,%n_step,%c32
                ...
                %vv = cooperative_matrix_load.n %v[%i0,%n_vp] : coopmatrix<f16x32x32,matrix_a>
                %pp1 = cooperative_matrix_load.n %P_sub_m[%c0,%c0] : coopmatrix<f16x32x32,matrix_b>
        ...
        cooperative_matrix_store %o_scaled_f16, %o[%i0,%j0]
    }
}
...
; %V: (d,N,H,B)
; %O: (d,N,H,B)
func @flash_attention(%Q: memref<f16x64x?x?x?,strided<1,?,?,?>> {stride_gcd=[1,64,64,64]},
        ...
        for %j=%j0,%block_size,%c512 {
            %o_acc1,%o_acc2,%maxvec,%normvec = for %n=%c0,%seq_len,%c32
                ...
                %shat = for %m=%c0,%c64,%c32 init(%s_iter=%s_init) -> (coopmatrix<f32x32x16,matrix_acc>) {
                    ...
                    ;%qq = cooperative_matrix_load.n.cols_checked %q[%m,%j] : coopmatrix<f16x32x16,matrix_b>
                    %kk = cooperative_matrix_load.t %k[%m,%n] : coopmatrix<f16x32x32,matrix_a>
                    %qq = cooperative_matrix_load.n %q[%m,%j] : coopmatrix<f16x32x16,matrix_b>
                ...
                ;%vv1 = cooperative_matrix_load.n.cols_checked %v[%c0,%n] : coopmatrix<f16x32x32,matrix_a>
                ;%vv2 = cooperative_matrix_load.n.cols_checked %v[%c32,%n] : coopmatrix<f16x32x32,matrix_a>
                %vv1 = cooperative_matrix_load.n %v[%c0,%n] : coopmatrix<f16x32x32,matrix_a>
                %vv2 = cooperative_matrix_load.n %v[%c32,%n] : coopmatrix<f16x32x32,matrix_a>
